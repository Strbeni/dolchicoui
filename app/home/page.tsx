  ...
  subCategories: cat.subCategories.map((sub, subIndex) => ({
    ...
    image:
      availableCategoryImages[
        (catIndex * 3 + subIndex) % availableCategoryImages.length
      ],
...
export default function Home() {
  ...
  const categoryIcons = [
    ...
    {
      name: "Boys",
      icon: "https://images.unsplash.com/photo-1503919545889-aef636e10ad4?w=80&h=80&fit=crop&crop=face",
    },
    {
      name: "Infant",
    ...
    <>
      <div className="hidden md:block">
        ...
        <section className="px-6 lg:px-20 pt-10 pb-8">
          ...
          <p className="text-center text-gray-600 mt-3 max-w-3xl mx-auto">
            Discover fashion that reflects your values and your style.
            Sustainably sourced, thoughtfully designed, endlessly stylish.
          </p>
            ...
            <div className="relative h-[120px] sm:h-[150px] rounded-2xl overflow-hidden">
              ...
              <div className="absolute inset-0 bg-gradient-to-t from-black/50 to-transparent p-2 sm:p-4 flex items-end">
                <div>
                  ...
                  <span className="text-white text-[10px] underline">
                    Explore Now →
                  </span>
                </div>
              </div>
      ...
      <section className="px-4 sm:px-6 lg:px-20 mt-8 mb-8">
        <div className="mb-4">
          ...
          <p className="text-sm text-gray-600">
            Style, inspired by the future of fashion
          </p>
        </div>
        <div className="space-y-6">
          {categoriesByCategory.slice(0, 5).map((categoryGroup) => {
            ...
            const displayedSubCategories = isExpanded
              ...
              : categoryGroup.subCategories.slice(0, 6); // Show 6 initially, all when expanded
                ...
                <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-4 lg:grid-cols-6 gap-3 md:gap-4">
                  {displayedSubCategories.map((item) => {
                    return (
                      <Link
                        key={item.id}
                        href={`/productlist?category=${encodeURIComponent(
                          categoryGroup.categoryName
                        )}&subcategory=${encodeURIComponent(item.title)}`}
                      >
                        <Card className="bg-transparent border-none shadow-none p-0 hover:shadow-lg transition-shadow duration-300">
                          ...
                          <CardContent className="px-1 md:px-2 pt-2 pb-1 md:pt-3 md:pb-2">
                            <p className="text-xs md:text-sm text-gray-700 font-medium text-center truncate">
                              {item.title}
                            </p>
                          </CardContent>
                        </Card>
