  ...
  useEffect(() => {
    const fetchProducts = async () => {
      ...
      try {
        ...
        const apiUrl = `${API_BASE}/api/product/list${
        ...
        if (data.success && Array.isArray(data.products)) {
          ...
          const transformedProducts: Product[] = data.products.map(
            (product: any, index: number) => ({
              ...
              subCategory:
                product.subcategory || product.subCategory || "Topwear",
          ...
          const offerType = JSON.parse(
            sessionStorage.getItem("currentOfferTypeFilters") || "[]"
          ) as {
            price_below?: number;
            price_above?: number;
            min_discount?: number;
          ...
          console.log(
            "OfferType from sessionStorage:",
            JSON.stringify(offerType, null, 2)
          );
          console.log(
            "Products count before filter:",
          ...
          const filtered = transformedProducts.filter((product) => {
                ...
                ? product.tags.some((tag) =>
                  ...
                  (offerType[0]?.subCategoriesName &&
                    product.subCategory.toLowerCase() ===
                      offerType[0].subCategoriesName.toLowerCase())
                : true;
            ...
            if (offerTag && offerType.length > 0 && offerType[0]) {
              ...
              if (
                ...
                console.warn(
                  "Invalid price range: price_above > price_below, bypassing price filter."
                );
                matchOfferType = true;
              } else {
                matchOfferType =
          ...
          if (offerTag) {
            ...
            prioritized = filtered.filter((product) =>
              product.tags.some(
                (tag) => tag.toLowerCase() === offerTag.toLowerCase()
              )
            );
            ...
            others = filtered.filter((product) => {
              const matchesOtherTags =
                offerType[0]?.tags?.some(
                  (tag) =>
                    product.tags.includes(tag) &&
                    tag.toLowerCase() !== offerTag.toLowerCase()
                ) || false;
