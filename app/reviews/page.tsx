...
const STATIC_ORDERED_PRODUCTS: OrderedProduct[] = [
    {
        id: 1,
        product: {
            id: 101,
        ...
        review: {
            ...
            productId: 101,
            rating: 4,
            comment: "Great quality t-shirt, very comfortable to wear.",
    ...
    },
    {
        ...
        review: {
            ...
            productId: 103,
            rating: 5,
            comment: "Excellent quality shirt, fits perfectly and looks professional.",
    ...
    },
    {
        ...
        review: {
            ...
            productId: 105,
            rating: 3,
            comment: "Good jacket but could be warmer for the price.",
    ...
    },
    {
        ...
        product: {
            ...
            description: "Stylish polo shirt for casual and semi-formal occasions",
            price: 1599,
            image: ["/product.jpg", "/p6.svg"],
            category: "Men",
            subCategory: "Polo",
            sizes: ["S", "M", "L", "XL"],
            bestseller: false,
    ...
    },
    {
        ...
        review: {
            ...
            productId: 107,
            rating: 4,
            comment: "Nice wallet, good quality leather and stitching.",
...
export default function ReviewsPage() {
    ...
    // Fetch ordered products (currently using static data)
    const fetchOrderedProducts = useCallback(async () => {
        try {
            ...
            try {
                // Try API call first
                const response = await fetch(`${API_BASE_URL}/api/orders/products/user`, {
                    method: 'GET',
                ...
                if (result.success) {
                    setOrderedProducts(result.data?.products || []);
                    return;
                }
            } catch (apiError) {
                console.warn('API call failed, falling back to static data:', apiError);
                // Fall back to static data if API fails
                setOrderedProducts(STATIC_ORDERED_PRODUCTS);
            }
        } catch (err) {
    ...
    useEffect(() => {
        ...
        if (filter === "reviewed") {
            filtered = filtered.filter(item => item.review);
    ...
    const handleLeaveReview = (productId: number) => {
        const product = orderedProducts.find(item => item.product.id === productId)?.product;
        if (product) {
    ...
    const handleEditReview = (productId: number) => {
        const orderItem = orderedProducts.find(item => item.product.id === productId);
        if (orderItem && orderItem.review) {
            setSelectedProduct(orderItem.product);
            setSelectedReview(orderItem.review);
            setIsEditMode(true);
            setShowReviewForm(true);
            setReviewRating(orderItem.review.rating);
            setReviewHeading(""); // Assuming heading is not stored in current review structure
            setReviewComment(orderItem.review.comment);
            setUploadedFiles([]);
        }
    };
    ...
    const handleSubmitReview = async () => {
        if (!selectedProduct || reviewRating === 0 || !reviewComment.trim()) {
        ...
        try {
            ...
            toast(`Failed to ${isEditMode ? 'update' : 'submit'} review. Please try again.`, false);
        } finally {
                    ...
                    <div className="max-w-7xl mx-auto flex gap-8">
                        <div className="w-1/4 flex-shrink-0">
                            <div className="sticky top-6">
                                <ProfileSidebar activeSection="reviews" />
                            </div>
                        </div>
                            ...
                            <div className="flex items-center justify-center h-64">
                                <div className="text-center max-w-md">
                                    ...
                                    <div className="space-y-3">
                                        <Button onClick={fetchOrderedProducts} className="w-full bg-orange-600 hover:bg-orange-700 text-white">
                                            Try Again
                                        </Button>
                            ...
                            <div className="bg-white rounded-lg shadow-sm lg:bg-transparent lg:shadow-none">
                                ...
                                <div className="p-4 lg:p-0">
                                    ...
                                    <div className="mb-8 hidden lg:block">
                                        <h1 className="text-3xl font-bold text-gray-900 mb-6">{isEditMode ? 'Edit review' : 'Add review'}</h1>

                                        {/* Product Info */}
                                        <div className="flex items-center gap-4 mb-6">
                                            <div className="w-16 h-16 relative shrink-0">
                                                <Image
                                                    src={selectedProduct?.image[0] || '/placeholder.png'}
                                                    ...
                                                    <div className="px-0 py-4 lg:py-6">
                                                        <div className="flex items-center gap-3 lg:gap-6">
                                                            ...
                                                            <div className="flex items-center gap-2 lg:gap-3 shrink-0">
                                                                {item.review ? (
                                                                    <button
                                                                        onClick={() => handleEditReview(item.product.id)}
                                                                    ...
                                                                    >
                                                                        <div className="flex gap-0.5 lg:gap-1">
                                                                            {[1, 2, 3, 4, 5].map((star) => (
                                                                                <Star
                                                                                    key={star}
                                                                                    className={`w-3 h-3 lg:w-4 lg:h-4 ${star <= item.review!.rating ? 'text-orange-400 fill-current' : 'text-gray-300'}`}
                                                                                />
                                                                            ))}
                                                                        </div>
                                                                    ...
                                                                    <Button
                                                                        onClick={() => handleLeaveReview(item.product.id)}
                                                                        size="sm"
                                                                    ...
                                                                    >
                                                                        <span className="sm:inline">Leave a review</span>
                                                                    </Button>
                                                                )}
                                                            </div>
                                                        </div>
